...
def test_main(num_tokens: int,
    ...
    avg_t, min_t, max_t = bench(partial(test_func, return_recv_hook=False), num_tests=num_tests, print_trace=print_trace, trace_file=trace_file)
    print(
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    try:
        test_main(num_tokens,
                  ...
                  use_dist_barrier=args.use_dist_barrier,
                  seed=1,
        ...
        for seed in range(int(1e9) if do_pressure_test else 0):
            ...
            for _ in range(20):
                assert test_main(num_tokens,
                                 ...
                                 num_tests=args.num_tests,
                                 print_trace=args.print_trace,
                                 trace_file=trace_file) == ref_hash, f'Error: seed={seed}'
    finally:
        if trace_file:
            trace_file.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--use-dist-barrier", action='store_true', help='Use dist.barrier(group=group) instead of group.barrier()')
    parser.add_argument('--num-tests', type=int, default=30, help='Number of tests for benchmarking (default: 30)')
